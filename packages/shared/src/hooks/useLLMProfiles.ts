...
import { llmProfilesApi } from "../api/client";
 ...
 * This mutation does NOT invalidate any cache â€” it is a read-only test call.
 */
export function useTestLLMProfile() {
  return useMutation({
    mutationFn: ({ id, body }: { id: string; body?: LLMTestRequest }) =>
      llmProfilesApi.test(id, body),
    // No cache invalidation needed â€” purely a connectivity test
    retry: 0,
  });
}
