...
export type TestCase = { id: string; name: string; project_id: string; target_type: string; revision: string; specification: Record<string, unknown> };
export type Artifact = { id: string; kind: string; uri: string; checksum: string; size: number; content_type: string | null };
export type Run = { id: string; correlation_id: string; status: string; revision: string; project_id: string; test_case_id: string; test_case_name: string | null; target_type: string | null; target_url: string | null; artifact_policy: { trace_on_failure: boolean; video_on_failure: boolean; screenshot_on_failure: boolean; trace_on_success: boolean; video_on_success: boolean; screenshot_on_success: boolean; retain_days: number } | null; playwright_test_source: string | null; blocked_external_origins: string[]; terminal_summary: string | null; created_at: string | null };
...
export type BrowserAction = { sequence: number; action: string; element: string; duration_ms: number | null; source_line: number | null; has_before_frame: boolean; has_after_frame: boolean };
...
export type RevisedDraftRequest = { id: string; correlation_id: string; state: "queued" | "generating" | "completed" | "failed"; failure_reason: string | null };
...
export const renameTestCase = (apiUrl: string, projectId: string, testCaseId: string, name: string) => apiRequest<TestCase>(apiUrl, `/api/v1/projects/${projectId}/tests/${encodeURIComponent(testCaseId)}/name`, { method: "PUT", body: JSON.stringify({ name }) });
...
export const browserActions = (apiUrl: string, id: string) => apiRequest<BrowserAction[]>(apiUrl, `/api/v1/runs/${id}/browser-actions`);
export const browserActionFrameUrl = (apiUrl: string, runId: string, sequence: number, frame: "before" | "after") => `${apiUrl}/api/v1/runs/${runId}/browser-actions/${sequence}/frames/${frame}`;
...
export const createRevisedDraft = (apiUrl: string, id: string) => apiRequest<RevisedDraftRequest>(apiUrl, `/api/v1/runs/${id}/revised-draft`, { method: "POST", headers: { "Idempotency-Key": idempotencyKey() } });
