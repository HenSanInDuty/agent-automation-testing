...
export default function RunDetailPage() {
  ...
  return <>
    ...
    <section className="workspace-section"><div className="section-heading"><h2>AI post-run review</h2>{report && <StatusBadge status={report.status} />}</div>{report?.status === "completed" && report.payload ? <><p>The agent reviews every terminal result, including PASS. It cannot change the deterministic verdict.</p><h3>{report.payload.headline}</h3><p>{report.payload.what_ran}</p>{report.payload.observations.length > 0 && <><h3>Verified observations</h3><ul className="stack-list">{report.payload.observations.map((observation, index) => <li key={`${observation.text}-${index}`}>{observation.text}{observation.evidence_references.length > 0 && <> — {evidenceLinks(observation.evidence_references)}</>}</li>)}</ul></>}{report.payload.failure && <><h3>Failure detail</h3><ul className="detail-list"><li>Stage: {report.payload.failure.stage}</li><li>Location: {report.payload.failure.location}</li><li>Reason: {report.payload.failure.message}</li>{report.payload.failure.evidence_references.length > 0 && <li>Evidence: {evidenceLinks(report.payload.failure.evidence_references)}</li>}</ul></>}{report.payload.unverified_or_skipped.length > 0 && <><h3>Unverified or skipped scope</h3><ul className="stack-list">{report.payload.unverified_or_skipped.map((item) => <li key={item}>{item}</li>)}</ul></>}{report.payload.limitations.length > 0 && <><h3>Limitations</h3><ul className="stack-list">{report.payload.limitations.map((item) => <li key={item}>{item}</li>)}</ul></>}<p>Provenance: {report.provenance.provider ?? "provider unavailable"}{report.provenance.model ? ` / ${report.provenance.model}` : ""}; prompt {report.prompt_version}.</p></> : report?.status === "unavailable" ? <p>The AI post-run review is unavailable: {report.unavailable_reason ?? "No safe reason was recorded."} The deterministic runner result remains the primary verdict.</p> : cancellable ? <p>Review will be prepared after a deterministic result.</p> : reportMissing ? <p>No review is available for this historical run.</p> : <p>Review will be prepared after a deterministic result.</p>}</section>
